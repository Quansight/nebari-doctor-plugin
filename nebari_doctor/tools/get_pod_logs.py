...
def get_nebari_pod_logs_tool(pod_names: list[str], since_minutes: int=10, namespace: str = "dev") -> dict:
    """
    ...
    """
    print(f'Getting pod logs for {",".join(pod_names)} for last {since_minutes} minutes')
...
def get_pods(namespace: str) -> str:
    ...
    pod_names = '\n'.join([f"{p['metadata']['name']} {p['status']['phase']}" for p in pod_dict['items']])
    return pod_names

if __name__ == '__main__':
    config_filepath = pathlib.Path('/home/balast/CodingProjects/nebari/scratch_deployment/nebari-config.yaml')
    namespace = 'nebari'
